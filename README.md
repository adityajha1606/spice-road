...
- **Docs** - `docs/SETUP-GUIDE.md`, `docs/DESIGN.md`, `docs/CUSTOMIZE.md`: the manual, the manifesto, and the how-to-make-it-yours guide.
...
That's the whole rule, and it's why the prompt is three rows instead of one line, and why there's a full boot sequence before you even get a cursor. `docs/DESIGN.md` has the longer argument, if you want it.

---

## Behind the Build

This cockpit wasn't assembled from a template. It was built live - real stress-testing on WSL2, debugging actual font-name mismatches, wrestling with kernel headers that wouldn't compile, and carefully extracting the weather fetch into a separate script when a background-job message started leaking into the prompt. The battery module uses a lock-file specifically to prevent concurrent PowerShell calls from stacking up across multiple tabs. The fastfetch config went through multiple iterations until a prebuilt `.deb` replaced a source build that refused to work on WSL2's stripped-down kernel. A health check script, `validate.sh`, emerged from a real bug found during review - every check now reports ✓ or ✗ explicitly, because the earlier version silently skipped failures.

Those details aren't glamorous, but they're the reason the cockpit feels like a precision instrument instead of a decoration.
...
- [x] Pre‑fetch hook for battery and weather caches (`spice-prefetch.sh`)
- [x] Health check script that never silently skips failures (`validate.sh`)
- [ ] Kubernetes context pill for the prompt
- [ ] Custom eza / bat theme files matching the palette
- [ ] Battery module with low‑battery colour threshold
- [ ] systemd timer for battery cache (optional, needs `systemd=true` in WSL config)
- [ ] GIF demo walkthrough

Something here you'd like to build? PRs are welcome — `docs/DESIGN.md` has the colour philosophy and design rules if you want to keep things cohesive.
...
[![WSL2](https://img.shields.io/badge/WSL2-subsystem-14100D?style=flat&logo=linux&logoColor=white)](#)
...
From there, `docs/SETUP-GUIDE.md` covers the rest end to end - including fastfetch, which ships as a prebuilt `.deb` in the guide, so there's no source build involved. Or just run `./install.sh`.

---

## Make It Yours

`docs/CUSTOMIZE.md` has the full guide, but three small changes give a sense of how it's wired:
...
Fork it, branch it, open a pull request. Shell scripts get linted on commit through `.pre-commit-config.yaml` - install the hook before you push anything. For architecture and design rationale, `docs/DESIGN.md` is the place to start.
